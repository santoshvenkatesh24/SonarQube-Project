pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'santosh149'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/santoshvenkatesh24/SonarQube-Project.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=Santosh -Dsonar.projectKey=SantoshKey -Dsonar.java.binaries=target"
                }
            }
        }
